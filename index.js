...
var mysql = require('mysql');
    ...
    var database_contact = mysql.createConnection({
      host: 'localhost',
      user: 'root',
      password: '',
...
const connect_wppaccount = async (req, res, callback, errcallback) =>  {
    ...
    if (sock.state !== "open" && sock.state !== "connecting") {
        ...
        sock.ev.on("open", () => {
          ...
          sock.ev.on('chat-update', onReceiveMessage);
                ...
                responseMessages.forEach(async response => {
                    response.keys.forEach(async key => {
                        if (message.message.conversation.toLowerCase().includes(key.toLowerCase())) {
                            ...
                            await sock.chatRead(chatUpdate.jid)
                        }
                    })
                })
            });
    ...
    if (sock) {
            getUsers(async users => {
                ...
                for (i = 0; i < users.length; i++) {
                    ...
                    sock.sendMessage (id, { text: user.message })
            .then(async() => { 
                sendedMessage = true
            ...
            database_contact.connect(function(err) {
                if (err) throw err;
                console.log("Connected!");
                var users = (id, phone,message ,data , status) => {
                    database_contact.query(`INSERT INTO contacts (id, phone, message,data,status) VALUES ('${user.id}','${user.phone}','${user.message}','${user.data}','505')`)
                }
                database_contact.query(users, function (err, result) {
                  if (err) throw err;
